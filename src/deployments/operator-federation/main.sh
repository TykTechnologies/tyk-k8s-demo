...
sed "s/replace_run_as_user/$applicationSecurityContextUID/g" "$deploymentPath/users-subgraph-app-template.yaml" | \
...
sed "s/replace_run_as_user/$applicationSecurityContextUID/g" "$deploymentPath/posts-subgraph-app-template.yaml" | \
  sed "s/replace_run_as_group/$applicationSecurityContextUID/g" | \
  kubectl apply --namespace "$namespace" -f - > /dev/null;
...
sed "s/replace_run_as_user/$applicationSecurityContextUID/g" "$deploymentPath/comments-subgraph-app-template.yaml" | \
  sed "s/replace_run_as_group/$applicationSecurityContextUID/g" | \
  kubectl apply --namespace "$namespace" -f - > /dev/null;
...
sed "s/replace_run_as_user/$applicationSecurityContextUID/g" "$deploymentPath/notifications-subgraph-app-template.yaml" | \
  sed "s/replace_run_as_group/$applicationSecurityContextUID/g" | \
  kubectl apply --namespace "$namespace" -f - > /dev/null;
