...
kubectl wait pods --namespace "$namespace" -l app=users-subgraph --for=condition=Ready --timeout="$TYK_TIMEOUT" > /dev/null;
...
kubectl wait pods --namespace "$namespace" -l app=posts-subgraph --for=condition=Ready --timeout="$TYK_TIMEOUT" > /dev/null;
...
kubectl wait pods --namespace "$namespace" -l app=comments-subgraph --for=condition=Ready --timeout="$TYK_TIMEOUT" > /dev/null;
...
kubectl wait pods --namespace "$namespace" -l app=notifications-subgraph --for=condition=Ready --timeout="$TYK_TIMEOUT" > /dev/null;
