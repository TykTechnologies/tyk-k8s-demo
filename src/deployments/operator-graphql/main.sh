...
kubectl wait pods --namespace "$namespace" -l app=users-graph --for=condition=Ready --timeout=60s  > /dev/null;

# Posts Graph
addService "posts-graph-svc";
kubectl apply --namespace "$namespace" -f "$operatorGQLDeploymentPath/posts-graph-app.yaml" > /dev/null;
kubectl wait pods --namespace "$namespace" -l app=posts-graph --for=condition=Ready --timeout=60s  > /dev/null;
...
kubectl wait pods --namespace "$namespace" -l app=comments-graph --for=condition=Ready --timeout=60s  > /dev/null;
...
kubectl wait pods --namespace "$namespace" -l app=notifications-graph --for=condition=Ready --timeout=60s  > /dev/null;
