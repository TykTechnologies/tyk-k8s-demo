...
kubectl wait pods --namespace "$namespace" -l app=users-rest --for=condition=Ready --timeout="$TYK_TIMEOUT" > /dev/null;
...
kubectl wait pods --namespace "$namespace" -l app=posts-rest --for=condition=Ready --timeout="$TYK_TIMEOUT" > /dev/null;
...
kubectl wait pods --namespace "$namespace" -l app=comments-rest --for=condition=Ready --timeout="$TYK_TIMEOUT" > /dev/null;
