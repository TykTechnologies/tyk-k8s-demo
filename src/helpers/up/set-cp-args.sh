...
port=$(kubectl get "service/mdcb-svc-$tykReleaseName-tyk-mdcb" --namespace "$namespace" -o jsonpath="{.spec.ports[0].port}");

ip="mdcb-svc-$tykReleaseName-tyk-mdcb.$namespace.svc";
if [[ $LOADBALANCER == "$expose" ]]; then
  ip=$(kubectl get "service/mdcb-svc-$tykReleaseName-tyk-mdcb" --namespace "$namespace" -o jsonpath="{.status.loadBalancer.ingress[0].ip}");
